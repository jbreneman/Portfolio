...
function getAnimationElements(id) {
    var idElement = document.getElementById(id);

    return {
        container: idElement,
        header: idElement.getElementsByClassName(id + '__header')[0],
        ele: idElement.getElementsByClassName(id+ '__section')
    }
}

function fadeIn(elements) {
    elements.header.classList.add('fadeInSideways');

    for(var i = 0; i < elements.ele.length; i++) {
        elements.ele[i].classList.add('fadeInSideways');
    }
}

var skills = getAnimationElements('skills'),
...
setInterval(function() {
    if (didScroll) {
        ...
        if(!skillsInView) {
            ...
            //if they are, add animation classes
            if(skillsInView) {
                fadeIn(skills);
            }
        }
         ...
         if(!projectsInView) {
            //check if divs are within viewport
            var projectsInView = withinviewport(projects.container, {bottom: -100});
            //if they are, add animation classes
            if(projectsInView) {
                fadeIn(projects);
            }
        ...
        if(!contactInView) {
            //check if divs are within viewport
            var contactInView = withinviewport(contact.container, {bottom: -100});
            //if they are, add animation classes
            if(contactInView) {
                fadeIn(contact);
            }
